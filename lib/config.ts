...
export function parseConfig(rawConfig: RawConfig): Readonly<Config> {
  ...
  if (
    !rawConfig.box.globalStatusOutputsByName !== !rawConfig.box.outputsByStatus
  )
    throw new TypeError(
      ...
      switch (rawConfig.box.lightSensor.transform?.function) {
        case 'LINEAR':
          brightnessTransform = {
            ...
            offset: _.defaultTo(rawConfig.box.lightSensor.transform.offset, 0),
  ...
  const inputsByName: Map<string, InputConfig> | undefined =
    ...
    new Map(
      Object.entries(rawConfig.box.inputsByName).map(
        ([name, input]): [string, InputConfig] => {
  ...
  const globalStatusOutputsByName: Map<string, OutputConfig> | undefined =
    getOutputMap(rawConfig.box.globalStatusOutputsByName)
  const localStatusOutputsByName: Map<string, OutputConfig> | undefined =
    getOutputMap(rawConfig.box.localStatusOutputsByName)
          ...
          return [
            ...
            new Map(
              Object.entries(outputValues).map(([name, value]) => {
                const output = globalStatusOutputsByName?.get(name)
                if (output === undefined)
                ...
                if (value.function === 'CONSTANT') {
                  ...
                  outputTransform = {
                    ...
                    coefficient: _.defaultTo(value.coefficient, 1),
                    offset: _.defaultTo(value.offset, 0),
                    min: _.defaultTo(value.min, 0),
                    max: _.defaultTo(value.max, output.range),
                  }
                } else {
                  throw new TypeError(
